...
def crossValidation_SVM(K, DTR, LTR, kappa, C, pi, pit):
...
def crossValidation_SVM_poly(K, DTR, LTR, kappa, C, degree, c, pi, pit):
    minDCFvalues1 = []
    minDCFvalues3 = []
    minDCFvalues5 = []
    N = DTR.shape[1]
...
def crossValidation_SVM_RBF(K, DTR, LTR, kappa, constrain, gamma, pi, pit):
    N = DTR.shape[1]
    seglen = N//K
    allscores = []
    for i in range(K):
...
def SVM_2C(feat, lab, K, C, pit):
    """ returns w, b to be used for classification w.T . x + b, given K(higher means less bias regularization) and C(hyperparam) """
...
def SVM_2C_ndegree(feat, lab, K, C, degree, c, pit):
    """ returns alpha, z, feat to be used for classify_SVM_2C_ndegree, given K(higher means less bias regularization), C(hyperparam) and the degree of the curve"""
    x0 = np.zeros(feat.shape[1])
    bnds = np.array([(0, C)] * feat.shape[1])
    TL = np.array(lab)
    TL = TL.reshape(-1, 1)
...
class SVMClass:
    ...
    def duality_gap(self, a, w_cap, C):
        ...
        print("primal loss: ", (0.5 * np.dot(w.T, w) + C* (np.array([ 0 if el < 0 else el for el in arr ]).sum())))
...
if __name__ == "__main__":
    ...
    minDCFvalues1F, minDCFvalues3F, minDCFvalues5F  = crossValidation_SVM(k, full_train_att, full_train_label, kappa, initial_C, pi, pit)
    ...
    minDCFvalues1Z, minDCFvalues3Z, minDCFvalues5Z = crossValidation_SVM(k, z_data, full_train_label, kappa, initial_C, pi, pit)
    ...
    minDCFvalues1Q, minDCFvalues3Q, minDCFvalues5Q  = crossValidation_SVM_poly(k, full_train_att, full_train_label, kappa, initial_C, initial_d, 1, pi, pit)
    ...
    minDCFvalues1Qz, minDCFvalues3Qz, minDCFvalues5Qz  = crossValidation_SVM_poly(k, z_data, full_train_label, kappa, initial_C, initial_d, 1, pi, pit)
    ...
    for g in gamma:
        for C in initial_C:
            minDCF =crossValidation_SVM_RBF(k, full_train_att, full_train_label, kappa, C, g, pi, pit)
            if g == 10**-1:
    ...
    for g in gamma:
        for C in initial_C:
            minDCF =crossValidation_SVM_RBF(k, z_data, full_train_label, kappa, C, g, pi, pit)
            if g == 10**-1:
