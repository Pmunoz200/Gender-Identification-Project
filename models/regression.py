...
def vrow(arr):  # if arr is a column n*m, m=1 we want 1 row and n columns
    """makes array a numpy row"""
    return arr.reshape(1, arr.shape[0])


def vcol(arr):  # if arr is a row then shape[0] is the length
    """makes array a numpy column"""
    if (arr.shape.__len__() == 1):
        return arr.reshape(arr.shape[0], 1)
    return arr.reshape(arr.shape[1], 1)  # this is for 1-row matrices


def crossValidation_logreg(K, DTR, LTR, l_list, nclasses, model, pi, pit):
    minDCFvalues1 = []
    minDCFvalues3 = []
    minDCFvalues5 = []
    N = DTR.shape[1]
...
def linear_regression_2C(feat, lab, l, pit):
...
def expande_feature_space(data):
    """compute 2-deg feature expansion (also called Phi(x)) of the data"""
    rt = []
...
def expanded_feature_space(sample):
    """utility method called in expande_feature_space"""
    return vcol(np.concatenate((np.dot(sample, sample.T).ravel(), sample.ravel())))


def crossValidation_logreg_onepoint(K, DTR, LTR, l, nclasses, model, pi, pit):
    N = DTR.shape[1]
    seglen = N//K
    alls = []
    for i in range(K):
...
if __name__ == "__main__":
    ...
    for i in q:
        if i==1:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, feat_expanded, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==0:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, full_train_att, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==1:
    ...
    for i in q:
        if i==1:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, feat_expanded_z, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==0:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, z_data, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==1:
    ...
    for i in q:
        if i==1:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, feat_expanded_PCA, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==0:
                minDCFvalues1, minDCFvalues3, minDCFvalues5=crossValidation_logreg(k, att_PCA5, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i==1:
    ...
    for i in q:
        if i == 1:
            minDCFvalues1, minDCFvalues3, minDCFvalues5 = crossValidation_logreg(
                k, feat_expanded_PCA_z, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i == 0:
            minDCFvalues1, minDCFvalues3, minDCFvalues5 = crossValidation_logreg(
                k, att_PCA5_z, full_train_label, l_list, 2, linear_regression_2C, pi, pit)
        if i == 1:
            ...
            if i == 0:
                ...
                for p in pi:
                    minDCF = crossValidation_logreg_onepoint(
                        k, full_train_att, full_train_label, searchL, 2, linear_regression_2C, p, x)
                    tableKFold[cont].append(minDCF)
                for p in pi:
                    minDCF = crossValidation_logreg_onepoint(
                        k, z_data, full_train_label, searchL, 2, linear_regression_2C, p, x)
                    tableKFold[cont].append(minDCF)
            if i == 1:
                tableKFold.append([f"QLR {x}"])
                for p in pi:
                    minDCF = crossValidation_logreg_onepoint(
                        k, feat_expanded, full_train_label, searchL, 2, linear_regression_2C, p, x)
                    tableKFold[cont].append(minDCF)
                for p in pi:
                    minDCF = crossValidation_logreg_onepoint(
                        k, feat_expanded_z, full_train_label, searchL, 2, linear_regression_2C, p, x)
                    tableKFold[cont].append(minDCF)
