    ...
    for j in range(DTV.shape[1]):
        ...
        for i in range(num_classes):
            ccp.append(np.exp(log_likelihood(
                vectorize_column(DTV[:, j]), mus[i], sigmas[i])))
...
def tied_gaussian_classifier(DTR, LTR, DTV, num_classes, priors=0):
    ...
    for j in range(DTV.shape[1]):
        ...
        for i in range(num_classes):
            ccp.append(np.exp(log_likelihood(
                vectorize_column(DTV[:, j]), mus[i], SW)))
        S.append(vectorize_column(np.array(ccp)))
    S = np.hstack(S)
...
def run_gaussian_models(data, label, pi, K, steps):
    print('\nMVG')
    #print(K, data, label, 2, multivariate_gaussian_classifier, f"no PCA", pi)
    perform_cross_validation(
        K, data, label, 2, multivariate_gaussian_classifier, f"no PCA", pi)
    for i in range(steps):
        pt, _ = ML.PCA(data, 6-i)
        data_transformed = np.dot(pt, data)
        perform_cross_validation(
            K, data_transformed, label, 2, multivariate_gaussian_classifier, f"PCA{6-i}", pi)

    print('\nNaive Bayes')
    perform_cross_validation(
        K, data, label, 2, gaussian_naive_bayes, f"no PCA", pi)
    for i in range(steps):
        pt, _ = ML.PCA(data, 6-i)
        data_transformed = np.dot(pt, data)
        perform_cross_validation(
            K, data_transformed, label, 2, gaussian_naive_bayes, f"PCA{6-i}", pi)

    print('\nTied gaussian')
    perform_cross_validation(
        K, data, label, 2, tied_gaussian_classifier, f"no PCA", pi)
    for i in range(steps):
        pt, _ = ML.PCA(data, 6-i)
        data_transformed = np.dot(pt, data)
        perform_cross_validation(
            K, data_transformed, label, 2, tied_gaussian_classifier, f"PCA{6-i}", pi)

    print('\nNaive tied gaussian')
    perform_cross_validation(
        K, data, label, 2, tied_naive_bayes_classifier, f"no PCA", pi)
    for i in range(steps):
        pt, _ = ML.PCA(data, 6-i)
        data_transformed = np.dot(pt, data)
        perform_cross_validation(
            K, data_transformed, label, 2, tied_naive_bayes_classifier, f"PCA{6-i}", pi)
...
if __name__ == "__main__":
    ...
    run_gaussian_models(full_train_att, full_train_label, pi, K, steps)
    print('\n\ngaussian models z-normalization pi=0.1')
    run_gaussian_models(z_data, full_train_label, pi, K, steps)
