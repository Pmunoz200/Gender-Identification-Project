...
def load(filepath):
    """loads data in the classical form (features, labels) given the absolute filepath"""
    ...
    # stack all the vestors horizontally to create the matrix
...
################################### cross validation########################################################
def crossValidation_GMM(K, DTR, LTR, model, ng, alpha, threshold, psi, pi=0.5):
    N = DTR.shape[1]
    seglen = N//K
    allscores = []
    for i in range(K):
...
def dsmean(mat):  # mean of a dataset
    """returns the empiric mean of the given dataset"""
    return vcol(mat.mean(1))


def dscovmatrix(mat):
    """returns the empiric covariance matrix for the given dataset"""
    DC = mat - dsmean(mat)
    return np.dot(DC, DC.T) / mat.shape[1]


def vcol(arr):  # if arr is a row then shape[0] is the length
    """makes array a np column"""
...
def logpdf_GMM(X, gmm):  # compute log-density of gmm for samples X
    """X is a matrix of data, gmm is in the form [(w1, mu1, C1), (w2, mu2, C2), ...]"""
    M = len(gmm)
    S = []
    for i in range(M):
        S.append(logpdf_GAU_ND(X, gmm[i][1], gmm[i][2]) + np.log(gmm[i][0]))
    S = np.vstack(S)
    logdens = scipy.special.logsumexp(S, axis=0)  # shape: (N, )
    return logdens


def logpdf_GAU_ND(x, mu, C):  # compute log-density for a matrix of samples
    """returns log-density for a matrix x of samples, given its mean and covariance"""
    M = x.shape[0]
...
def GMM_classifier_full(DTR, LTR, ng, alpha, threshold, psi):
    """returns a set of gmm, one for each class, each with ng components"""
...
def LBG_ng_constrained(X, ng, alpha, threshold, psi, gmstart=False):
    """X is data, ng is number of gaussians (power of 2), alpha is hyperparameter"""
    if (gmstart == False):
        cov = dscovmatrix(X)
...
def GMM_EM_constrained(X, gmm, threshold, psi):
    ...
    while 1:
        ...
        if (ll_curr < ll_prev):
            # print('llr is decreasing, stopping')
            return gmm
...
def minimum_detection_cost(pi, cfn, cfp, scores, labels):
    """return the minimum normalized DCF for the given scores, labels and working point (iterating over all the possible thresholds)"""
...
def confusion_matrix(predictions, labels, nclasses):
    """return the confusion matrix where predictions are the predicted labels"""
...
def graph_data(raw_results, z_results, model, pca=0):
    """
    Plots a bar graph to visualize the minDCF values for raw and z-scored results.

    Parameters:
    raw_results (list): List of minDCF values for raw results.
    z_results (list): List of minDCF values for z-scored results.
    model (str): Name of the model.
    pca (int, optional): Number of principal components used for PCA. Defaults to 0.
...
if __name__ == "__main__":
    ...
    # z_values_pca = call_GMM(att_PCA5_z, full_train_label, pi=pi, models=headers)
    ...
    # z_values_d_pca = call_GMM(att_PCA5_z, full_train_label, pi=pi, models=headers)
    ...
    # z_values_t_pca = call_GMM(att_PCA5_z, full_train_label, pi=pi, models=headers)
    ...
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, full_train_att, full_train_label, "GMM full", 2, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, z_data, full_train_label, "GMM full", 2, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    print(tabulate(tableKFold, headers))    
    tableKFold.append("GMM diagonal 16")    
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, full_train_att, full_train_label,"GMM diagonal" , 16, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, z_data, full_train_label, "GMM diagonal", 16, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    
    tableKFold.append("GMM diagonal 4")        
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, full_train_att, full_train_label, "GMM diagonal", 4, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, z_data, full_train_label, "GMM diagonal", 4, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    
    tableKFold.append("GMM tied 32")    
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, full_train_att, full_train_label, "GMM tied", 32, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, z_data, full_train_label, "GMM tied", 32, 0.1, 0.000001, 0.01, p)
        tableKFold.append(minDCF)
    ...
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5, full_train_label, "GMM full", 8, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5_z, full_train_label, "GMM full", 8, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    
    tablePCA.append("GMM PCA5 diagonal 2")    
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5, full_train_label, "GMM diagonal", 2, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5_z, full_train_label, "GMM diagonal", 2, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    
    tablePCA.append("GMM PCA5 diagonal 16")    
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5, full_train_label, "GMM tied", 16, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5_z, full_train_label, "GMM tied", 16, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    
    tablePCA.append("GMM PCA5 tied 4")    
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5, full_train_label, "GMM tied", 4, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
    for p in pi:
        minDCF=minDCF = crossValidation_GMM(k_value, att_PCA5_z, full_train_label, "GMM tied", 4, 0.1, 0.000001, 0.01, p)
        tablePCA.append(minDCF)
